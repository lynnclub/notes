...
- **预处理**：
  - **清洗**：去除HTML标签、低质量文本、重复内容、敏感信息。
  - **分词（Tokenization）**：使用子词分词算法（如Byte Pair Encoding, BPE；SentencePiece）将文本转换为token序列，构建词表（通常几万到几十万token）。
  - **格式**：最终数据通常以二进制格式（如`.bin`、`.arrow`）或分片JSON存储，便于高效加载。

#### 权重文件与目录结构
- **权重文件**：训练得到的模型参数以特定格式保存。常见的有：
...
- **目录结构示例**（以HuggingFace格式为例）：
  ```
  model_name/
  ├── config.json            # 模型配置（层数、头数、维度等）
  ├── tokenizer.json         # 分词器配置
  ├── tokenizer_config.json
  ├── model.safetensors      # 模型权重（可多个分片）
  └── generation_config.json # 生成配置（如最大长度、采样策略）
  ```
...
Transformer 章节说过，LLM 只使用解码器，而解码器就是通过瞻前顾后完成“文字接龙”。

顾后使用掩码多头自注意力，在预测当前位置时，只能看到该位置之前（包括自身）的token，不能看到未来的token，保证生成的因果顺序正确。
...
DeepSeek-R1 通过大规模强化学习（RL）来训练模型学会“思考”。就像人类解题时会先在草稿纸上演算一样，模型被训练成在给出最终答案前，先生成内部的思考过程，即生成思维链（Chain of Thought）。在训练过程中，研究人员观察到非常有趣的现象——模型会出现“顿悟时刻”：它会主动重新评估自己的初步思路，推翻错误假设，寻找更优解法。这种自我反思和验证的能力，是通过强化学习自然涌现出来的，而不是被人为预设的。

...
![Untitled](Golang%2068e06f32c1404de2a6d5932835f20720/Untitled.png)

引用类型

![Untitled](Golang%2068e06f32c1404de2a6d5932835f20720/Untitled%201.png)

go语言没有引用传递，都是值传递。

![Untitled](Golang%2068e06f32c1404de2a6d5932835f20720/Untitled%202.png)
...
for range array、slice，循环前复制数据、统计，循环过程中增删元素不影响。

for range map，没有指定循环次数，使用指针，但是map底层是hash，循环过程中增删元素位置随机，无法确定影响。

for range channel，一直循环，如果有数据则取出，没有则阻塞，如果channel被关闭则退出循环。
...
[Go内存分配与回收](Golang%2068e06f32c1404de2a6d5932835f20720/Go%E5%86%85%E5%AD%98%E5%88%86%E9%85%8D%E4%B8%8E%E5%9B%9E%E6%94%B6%20a8b31f5e7150435e883b918633982b2a.md)
...
1. `sync.Mutex` 当使用一个初始化的锁，并未加锁就在代码中就解锁，会发生 throw 奔溃；
...
![Untitled](Golang%2068e06f32c1404de2a6d5932835f20720/Untitled%203.png)

20万个协程占用了500M内存，50万KB，平均一个协程占用2.5KB。

短时间CPU尖刺，可能是频繁io轮询，协程切换导致的，缩短协程超时时间可解决。
...
上面实现的 `isChanClose` 可能还是会出现 panic，因为判断有时间窗口。要避免出现 panic，本质上是保证事件的时序，官方推荐通过 `context` 来配合使用，我们可以通过一个 ctx 变量来指明 close 事件，而不是直接去判断 channel 的一个状态。
...
切片、Map线程不安全。并发写入时，切片会覆盖，map会报错`fatal error: concurrent map writes`。
...
方法Deadline、Done、Err、Value。

1. 控制并发任务的顺序、停止、超时。
2. 传递KV。

[**CGO问题**](Golang%2068e06f32c1404de2a6d5932835f20720/CGO%E9%97%AE%E9%A2%98%2019e315b90eee451d8d1d0abbef33161c.md)